...
public class StandLogo 
{
    private Mesh standLogo;
    private float x  = 0, y = 0, z = 0;
    
    public StandLogo(float width, float height, float depth)
    {
        BufferedImage img = null;
        try 
        {
            img = ImageIO.read(new File("./assets/map_data/bleachers/nintendo_logo.bmp"));
        } catch (IOException ex) 
        {
            ex.printStackTrace();
        }
        
        standLogo = new Mesh(Arrays.asList(new Triangle[]{
        //SOUTH
        ...
        standLogo.transform.setRotAngleY(degToRad(90));
    }
    
    public Mesh getStandLogo()
    {
        return this.standLogo;
    }
}
