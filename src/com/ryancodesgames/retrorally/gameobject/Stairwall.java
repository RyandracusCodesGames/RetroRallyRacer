...
{
    private Mesh stairwall;
    private float x  = 0, y = 0, z = 0;
    
    public Stairwall(float width, float height, float depth)
    {
        BufferedImage img = null;
        try 
        {
            img = ImageIO.read(new File("./assets/map_data/stairwell/Stairwall/stairwall.bmp"));
        } catch (IOException ex) 
        {
            ex.printStackTrace();
        }
        
        stairwall = new Mesh(Arrays.asList(new Triangle[]{
        //SOUTH
        ...
        stairwall.transform.setRotAngleY(degToRad(90));

    }
    
    public Mesh getStairwall()
    {
        return this.stairwall;
    }
}
